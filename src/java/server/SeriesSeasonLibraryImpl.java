...
public class SeriesSeasonLibraryImpl extends Object implements SeriesSeasonLibrary{

   private Hashtable<String,SeriesSeason> aLib;
   private static final String fileName="series.json";
   
   public SeriesSeasonLibraryImpl() {
   ...
   public boolean addSeriesSeason(SeriesSeason seasonSeriesToAdd){
   ...
   public boolean saveLibraryToFile(){
	try (FileWriter file = new FileWriter("series10.json")){
  ...
  //	SeriesSeasonLibraryImpl tempSLibrary = new SeriesSeasonLibraryImpl();
  // 	this.aLib = tempSLibrary.getHashTable();
  // 	return true;
  //}

   public boolean restoreLibraryFromFile(){
      System.out.print("restoreLibraryFromFile");
      boolean ret = true;
      try{
         aLib.clear();
  	 InputStream is = this.getClass().getClassLoader().getResourceAsStream(this.fileName);
