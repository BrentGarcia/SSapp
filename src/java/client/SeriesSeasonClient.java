...
TreeSelectionListener {
	...
	private SeriesSeasonLibraryImpl library;
	private SeriesSeasonLibraryImpl sLibrary;
	private SeasonTcpProxy sc;
	...
	public SeriesSeasonClient(String aHost, String aPort, String author, String authorKey) {
		...
		sc = (SeasonTcpProxy)new SeasonTcpProxy(host, Integer.parseInt(port));
	...
	public void rebuildTree(){
	  try{
		...
		// put nodes in the tree for all registered with the library
                ...
                String[] sTitleList = sc.getAllSeriesSeasonTitles();
		//FOR EVERY SERIES
		for (int i = 0; i<sTitleList.length; i++){
			
                        SeriesSeason aSS = sc.getSeriesSeason(sTitleList[i]);
			...
			for (int j = 0; j<sEpisodesList.length; j++){
			    ...
			    DefaultMutableTreeNode toAdd = new DefaultMutableTreeNode("Episode " + (j+1));
			...
			if(node!=null){
				...
				if(node.getChildCount()==0 &&
					...
					if (searched == true) {
						...
						ss = sc.getSeriesSeason(parentNodeLabel);
					}
					//extract Series Title from node label
						...
						ss = sc.getSeriesSeason(alteredParentNodeLabel);
					}
					...
					ratingJTF.setText(ss.imdbRating); // change to rating of the episode 
					...
					if (searched == true) {
						...
						ss = sc.getSeriesSeason(nodeLabel);
					} else {
						...
						//ss = library.getSeriesSeason(alteredNodeLabel);
					        ss = sc.getSeriesSeason(alteredNodeLabel);
					}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals("Exit")) {
			...
			sLibrary = new SeriesSeasonLibraryImpl();
		   ...
		   try{
			...
			SeriesSeason ssToRemove = sc.getSeriesSeason(nodeLabel);
			sc.removeSeriesSeason(ssToRemove);
			rebuildTree();
			tree.removeTreeSelectionListener(this);
                   }
		   catch (Exception ex) {
	...
	public static void main(String args[]) {
		...
		try{
			...
			SeriesSeasonClient src = new SeriesSeasonClient(host,port,name,key);
		}catch (Exception ex){
			ex.printStackTrace();
		}
	}
